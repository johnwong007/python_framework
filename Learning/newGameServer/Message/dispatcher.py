...
class Dispatcher:
    ...
    def _send(self, remotename, data, timeout=None):
        ...
        try:
            ...
            self.cabbit_server.send(remotename[1], data)
            return True
        except:
    ...
    def _send_group(self, remotename, data, timeout=None):
        ...
        try:
            if timeout == 0:
                timeout = None
            data = self.join_send_info(remotename,data)
            self.cabbit_server.send(remotename[0], data)
            return True
        except:
    ...
    def _connect_rbt_server(self):
        ...
        try:
            self.cabbit_server = cabbit.Client(server_url, "poker", self.my_q_name, arguments={'x-ha-policy':'all'})
            self.cabbit_server.connect()
            self.cabbit_group = cabbit.Client(server_url, "poker", self.my_grp_qname, arguments={'x-ha-policy':'all'})
            self.cabbit_group.connect()
